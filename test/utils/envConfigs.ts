...
function getPlatform(): OS {
  ...
  if (platform == null)
    throw new EnvironmentalError(
      `Platform is not provided. Example 'platform=android | ios'`
    );
...
function getBrandLocale(): [Brand, Locale] {
  ...
  if (br_lc == null)
    throw new EnvironmentalError(
      `Brand and locale is not provided. Example 'br_lc=DEMO:en_US'`
    );
  ...
  if (splits.length != 2 || splits[1].split("_").length != 2)
    throw new EnvironmentalError(
      `'${br_lc}' Brand / Locale is invalid. Example 'br_lc=DEMO:en_US'`
    );

  if (Locale[splits[1]] === undefined) {
    const listOfSupportedLocales = Object.values(Locale)
      .filter((value) => typeof value === "string")
      .toString();
    throw new EnvironmentalError(
      `'Locale is Unsupported. Supported Locales: ${listOfSupportedLocales}'`
    );
  }

  return [
    splits[0] as Brand,
    Locale[splits[1] as keyof typeof Locale] as Locale,
  ];
}
...
function getOSVersion(): string | never {
  ...
  if (version == null)
    throw new EnvironmentalError(
      "Specify OS version like DEVICE_VERSION='16.0' or '12 in Environmental Variables"
    );

  if (!parseFloat(version))
    throw new EnvironmentalError(
      `'${version}' OS vesion is invalid. Specify OS version like '16.0' or '12`
    );
...
export function getLocalizedStrings(locale: Locale) {
  ...
  try {
    return JSON.parse(fs.readFileSync(fileName).toString("utf-8")) as Record<
      string,
      string
    >;
  } catch (err) {
...
export const localizedStrings: Record<string, string> =
...
function getBuildPath(path: string, type: OS, brand: Brand): string {
  if (fs.existsSync(path)) {
    ...
    const buildName =
      type == OS.IOS && brand == Brand.KK ? "kphone" : (brand as string);
    const regex: string =
      type == OS.ANDROID
        ? `^DEMO-${buildName}[\\w-]*.[0-9-]*\\.apk$`
        : `^${buildName}\\.ipa$`;
    const matches: string[] = fs
      .readdirSync(path)
      .filter((allFiles: string) => allFiles.match(regex) !== null);
    ...
    throw new EnvironmentalError(
      `${path} does not exist! cwd: '${process.cwd()}'`
    );
}
...
function getEnv(key: string): string {
  ...
  if (value == null)
    throw new EnvironmentalError(
      `Could not Find ${key} in Environment Variables`
    );
  else return value;
}
